...
class GreetingAccount {
  text = '';
  constructor(fields: {text: string} | undefined = undefined) {
    if (fields) {
...
const GreetingSchema = new Map([
  [GreetingAccount, {kind: 'struct', fields: [['text', 'String']]}],
]);

/**
 * The expected size of each greeting account.
 */
// Let add a new class instance in order to increase the size allocated to the program
const sampleGreeter = new GreetingAccount();
sampleGreeter.text = '00000000000000000000';
...
export async function checkProgram(): Promise<void> {
  ...
  const GREETING_SEED = 'hello from Earth.';
...
export async function reportGreetings(): Promise<void> {
  ...
  console.log(greetedPubkey.toBase58(), 'receives ', greeting.text);
}
